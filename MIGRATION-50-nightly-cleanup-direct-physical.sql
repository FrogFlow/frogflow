...
-- fulfillment_at в будущем»), но на момент этой миграции для Direct-каналов
-- (Instagram/WhatsApp) чекаут физических заказов ещё не был сделан (Ниши,
-- Блок 8.3 — на тот момент сознательно вне объёма): createOrderFromCart
-- (direct-purchase.server.ts) никогда не заполнял fulfillment_at, он был
-- всегда NULL. Старое условие
...
--
-- [Обновление, ревизия кондитерской ветки, Блок 13, находка 13.6] Блок 8.3
-- позже реализован: createOrderFromCart с тех пор заполняет fulfillment_at
-- и для Direct-заказов тоже — абзац выше устарел именно в этом утверждении.
-- Сама SQL-защита ниже осталась верной и нужной без изменений: NULL и для
-- Direct-заказов остаётся законным «дата ещё не согласована» (например, до
-- MIGRATION-52/зон доставки), а не признаком канала, которого больше нет.
-- ═══════════════════════════════════════════════════════════════════════════
