...
function BroadcastPage() {
  ...
  async function onStart() {
    ...
    if (
      audienceType !== "test" &&
      !confirm(`Запустить рассылку для ~${audienceCount ?? "?"} получателей?`)
    )
      return;
    setBusy(true);
    try {
          ...
          {audienceType === "country" && (
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Фильтр по стране, которую пользователь выбрал в боте при первом входе. RU — Россия,
                KZ — Казахстан.
              </p>
            </div>
          )}
          ...
          {((activeBroadcast.data as any).status === "queued" ||
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={busy}
                onClick={async () => {
                  if (!confirm("Отменить рассылку? Неотправленные получатели будут пропущены."))
                    return;
                  setBusy(true);
                  try {
          ...
          return (
            <div key={b.id} className="bg-card border rounded-lg p-3 text-sm">
              ...
              <div className="text-muted-foreground">
                ...
                {b.failed_count > 0 && (
                  <span className="text-destructive ml-2">ошибки: {b.failed_count}</span>
                )}
                {b.blocked_count > 0 && (
                  <span className="ml-2">заблокировали: {b.blocked_count}</span>
                )}
              </div>
            </div>
          );
