  ...
  useEffect(() => {
    ...
    const collect = (root: ParentNode) => {
      ...
      for (let node = walker.nextNode() as Text | null; node; node = walker.nextNode() as Text | null) {
        ...
        if (!parent || parent.closest("input, textarea, select, option, script, style, code, pre, [data-no-translate]")) continue;
    ...
    const refresh = () => {
      ...
      if (locale === "ru") {
        nodes.forEach((node) => { const value = originals.get(node); if (value) node.data = value; });
