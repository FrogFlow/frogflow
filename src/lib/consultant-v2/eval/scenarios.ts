...
export const V2_EVAL_SCENARIOS: EvalScenario[] = [
  ...
  },
  {
    ...
    source: "BOVI 19.09",
    turns: [
      // 25.09, 13:20: «Здравствуйте. Что подсказать?» — на вопрос не ответил.
      { text: "Что у вас есть?", expect: { broad: true, must: [/постел|полотен|одеял|подуш/i] } },
  ...
  {
    id: "typos-mats-bedding",
    title: "Опечатки: «каврик в ванну», «пастельное бельё»",
    source: "сборный: опечатки на слух, как «Пальтеца» 24.09",
    turns: [
      {
        text: "Каврик в ванну есть?",
        expect: { mustNot: [/нет в (нашем )?ассортименте|в наличии нет|нет в наличии/i] },
      },
      RESET,
      {
        text: "Пастельное бельё семейное есть?",
        expect: { mustNot: [/нет в (нашем )?ассортименте|в наличии нет|нет в наличии/i] },
      },
    ],
  },
  ...
  },
  {
    ...
    turns: [
      // 25.09, прогон: «товаров производства Японии нет» — а Uchino японская.
      {
        ...
        // 25.09, 13:20: «Dorelan и Traumina (Италия)» — Traumina немецкая.
        expect: {
          mustNot: [
            /(?:нет|не представлен)[^.]*япон|япон[^.]*(?:нет|не представлен)/i,
            /Traumina[^.\n]{0,15}\(Итал/i,
          ],
        },
      },
      {
        text: "Можно 50x100 и 70x140 и 100x150",
        // «Weseta (Голландия)» — Weseta швейцарская.
        expect: { maxChars: 700, mustNot: [/Weseta[^.\n]{0,15}\(Голланд/i] },
      },
    ],
  },
  {
  ...
  {
    id: "moscow-delivery-prices",
    title: "Доставка в Москву, потом цены — сразу в рублях",
    source: "BOVI 27.09",
    turns: [
      {
        text: "Доброго дня. Подскажите стоимость доставки в Москву? И сроки.",
        expect: { must: [/СДЭК/i] },
      },
      // 27.09: страну модель не запомнила — три ответа клиентке из Москвы в тенге.
      { text: "Коврики японские для ванны 35*50 сколько стоят?", expect: { rubles: true } },
    ],
  },
  {
    id: "delivery-israel",
  ...
  // ── Живые вопросы BOVI, которых в наборе не было (разбор 25.09) ─────────
  {
    id: "family-bedding",
    title: "Семейный комплект — «где 2 одеяла»: два пододеяльника 155х200",
    source: "BOVI 23.09",
    turns: [
      { text: "Подскажите пожалуйста, цену семейного комплекта?" },
      // В прайсе «семейного» нет: это КПБ с двумя пододеяльниками 155х200.
      {
        text: "Размер где 2 одеяла",
        expect: { must: [/155\s*(?:[хx×*]|на)\s*200/i], mustNot: [/пудар|пародь|подод(?!еял)/i] },
      },
    ],
  },
  {
    id: "how-to-buy-big-towel",
    title: "«Как купить?», потом «нужно большое» — банные размеры",
    source: "BOVI 22.09",
    turns: [
      { text: "Здравствуйте. Как купить?" },
      {
        text: "Полотенце. Мне нужно большое",
        expect: {
          must: [/70\s*[хx×]\s*140|100\s*[хx×]\s*150|90\s*[хx×]\s*170|75\s*[хx×]\s*150|банн/i],
        },
      },
    ],
  },
  {
    id: "foot-towel",
    title: "Полотенце для ног",
    source: "BOVI 22.09",
    turns: [
      {
        text: "Полотенце для ног меня интересует",
        expect: {
          must: [
            /50\s*[хx×]\s*80|60\s*[хx×]\s*90|35\s*[хx×]\s*50|50\s*[хx×]\s*70|Uchino|Rivolta|Weseta/i,
          ],
        },
      },
    ],
  ...
  {
    id: "towel-size-names",
    title: "Размеры полотенец: 70х140 — не лицевое",
    source: "прогон 25.09: «для лица (70х140)»",
    turns: [
      {
        text: "Какие размеры полотенец есть?",
        expect: {
          mustNot: [
            /лиц\S*[^.?\n]{0,20}70\s*[хx×]\s*140/i,
            /70\s*[хx×]\s*140[^.?\n]{0,15}для\s+лиц/i,
          ],
        },
      },
  ...
  {
    id: "store-visit",
    title: "Адрес бутика и что там можно посмотреть",
    source: "BOVI 25.09",
    turns: [
      {
        text: "Добрый день! Как адрес вашего магазина? Что бы подьехать и посмотреть ассортимент?",
        expect: { must: [/Сатпаева/i] },
      },
      {
        text: "Подушки у Вас там представлены?",
        expect: { must: [/подушк|Traumina|Dorelan/i], mustNot: [/нет в наличии|в наличии нет/i] },
      },
    ],
  },
  {
    id: "diffuser",
    title: "Диффузор для дома — есть Cote Noire, а не «такого нет»",
    source: "BOVI 25.09: v1 без модели ответил невпопад",
    turns: [
      {
        text: "Добрый день, парфюмированный деффузатор для дома, можно цену уточнить",
        expect: {
          must: [/диффузор|Cote Noire/i],
          mustNot: [/нет в наличии|в наличии нет|не прода/i],
        },
      },
      { text: "Полотенца голландские сколько стоят?" },
      { text: "В тенге сколько?", expect: { rubles: false } },
    ],
  },
  {
    id: "aquanova-maks",
    title: "«А акванова Маск?» — это Aquanova Maks, а не «таких нет»",
    source: "тест v2 25.09",
    turns: [
      { text: "Акванова какие есть?" },
      {
        text: "А акванова Маск?",
        expect: { must: [/Maks/i], mustNot: [/нет в наличии|таких нет|в наличии нет/i] },
      },
    ],
  },
  {
    id: "aquanova-compare",
    title: "«Макс и Лондон в чем разница?» — факты из базы знаний, Maks не серия London",
    source: "тест v2 25.09",
    turns: [
      { text: "Коврики для ванной есть?", expect: { broad: true } },
      {
        text: "Акванова Макс и Лондон в чем разница?",
        expect: {
          must: [/хлоп|1200|египет|скольж/i],
          // Про Maks в базе ничего нет; «микрополиэстер, 2600 г/м²» — это Mauro.
          mustNot: [/LONDON[^.?\n]{0,60}60\s*[хx×]\s*60/i, /микрополиэстер|2600/i],
        },
      },
    ],
  },
  {
    id: "expected-colors",
    title: "«Яркие расцветки ожидаете?» — поставок не обещать",
    source: "BOVI 22.09",
    turns: [
      {
        text: "И ещё скажите, пожалуйста, яркие расцветки полотенец ожидаете? Зеленый, Бордо",
        expect: {
          handoff: "any",
          mustNot: [
            /поступ(?:ят|ит|ление)|ожида(?:ем|ется)|привез[её]м|скоро будут|на следующей неделе/i,
          ],
        },
      },
    ],
  ...
  {
    id: "order-with-phone",
    title: "«Оформляем» с телефоном и городом — менеджеру, покупка",
    source: "BOVI 21.09",
    turns: [
      { text: "Подушка Traumina Swing 50х70 есть?" },
      {
        text: "Оформляем. Мой номер +77002538888, Алматы",
        expect: { handoff: "purchase" },
      },
    ],
  },

  {
    id: "topper-height-waterproof",
    title: "Наматрасник на матрас 160х200х25, нешуршащий непромокаемый",
    source: "BOVI 27.09",
    turns: [
      {
        text: "Наматрасник для высокого матраса 160*200*25 есть? Нешуршаший непромокаемый",
        // 27.09: «Dorelan C.M.FIBERSAN 160х200, высота 32-37 см, непромокаемый» —
        // в данных «непромокаемый» нет, и на 25 см он велик.
        expect: { mustNot: [/непромокаем(?:ый|ая|ое)\s*[,.]/i] },
      },
    ],
  },
