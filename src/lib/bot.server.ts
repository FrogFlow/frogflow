...
async function sendProductCard(
  ...
  // Остаток считается на весь товар, не на отдельный вариант. NULL — учёт
  // остатка по этому товару не ведётся ("под заказ"), а не "нет в наличии":
  // `!== undefined` здесь было опечаткой (поле из Supabase всегда либо
  // число, либо null, никогда не undefined) — условие никогда не отсекало
  // NULL, и с включённым модулем stock ЛЮБОЙ товар без явно проставленного
  // остатка (типичный кейс для кондитерской "печём под заказ") показывался
  // как нераскупаемый: подпись "нет в наличии" и пустая клавиатура вместо
  // кнопки "В корзину" (addToCart и карточка в Mini App/Direct этой ошибки
  // не повторяют — там уже `!== null`).
  const outOfStock =
    (await hasModule("stock")) && p.stock_quantity != null && p.stock_quantity <= 0;
...
async function handleCallbackQuery(cq: TelegramCallbackQuery): Promise<void> {
  ...
  if (data.startsWith("lang_") && isLocale(data.slice(5).split(":")[0])) {
    ...
    if (materialOk) {
      const updatedDeliveredLanguage = addDeliveredLanguage(item.delivered_language, lang);
      await s
        .from("order_items")
        .update({ delivered_language: updatedDeliveredLanguage })
        .eq("id", item.id);

      // Учителя-HIGH: deliverOrder (orders.server.ts) сдвигает delivery_index
      // мимо позиции, ждущей выбора языка (иначе выдача зависла бы), и сам
      // не может узнать, что покупатель наконец ответил — эта проверка
      // видит только его событие. Заказ считается выданным, только если ни
