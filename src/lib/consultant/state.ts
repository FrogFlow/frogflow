...
export type ConsultantState = {
  ...
  v2_rub?: boolean;
  /**
   * Консультант v2: перед передачей заказа уже спросили телефон и город.
   * Второй раз не спрашиваем — не дал, значит не хочет; передаём как есть.
   */
  v2_contact_asked?: boolean;
  /**
   * Консультант v2: напоминание уже ушло после ответа бота с этим временем
   * (last_bot_reply_at). Одно напоминание на одну паузу покупателя.
   */
  v2_followup_for?: string;
  /**
   * Консультант v2: файлы фотобазы (пути), которые покупатель уже получил в
   * этом диалоге. Одно фото — на всю модель: второй раз его не шлём.
   */
  v2_media_sent?: string[];
};
