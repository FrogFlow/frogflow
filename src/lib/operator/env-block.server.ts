...
export async function buildEnvBlockFor(input: EnvBlockInput): Promise<EnvBlockResult> {
  ...
  const lines: string[] = [
    ...
    // печатать на семи живых деплоях, у которых её никогда не было. Но
    // молчаливое опущение не снимает VERTICAL, если она уже стоит в Vercel
    // (клиента переводят из другой ниши обратно в "Цифровые материалы") —
    // блок вставляют целиком, и старое значение просто продолжает
    // действовать (Блок 11, находка 11.4). Явный комментарий-инструкция
    // безвреден для деплоев, где переменной никогда не было (просто
    // пропускается при вставке, как и остальные "#"-строки этого блока), и
    // единственный явный сигнал для перехода назад.
    ...(vertical !== "digital"
      ? [`VERTICAL=${vertical}`]
      : [
          "# VERTICAL — если уже задана в Vercel (клиента переводят из другой ниши),",
          '# удалите её вручную: без неё деплой считается нишей "Цифровые материалы".',
        ]),
    "",
    "# ── Общая база ──",
