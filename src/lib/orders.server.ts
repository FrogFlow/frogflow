    ...
    for (let n = 0; n < BATCH_SIZE; n++) {
      ...
      try {
        if (availableLangs.length === 0) {
          // Ни одного файла ни на одном языке — товар без материалов
          // (Учителя, находка CRIT-1: addToCart с недавних пор такой товар
          // в корзину не пускает, но заказы, оформленные раньше правки, и
          // ручной ввод остаются). Раньше это молча считалось "sent":
          // покупатель платил, получал 0 файлов и сообщение "Заказ выдан",
          // продавец не видел ни следа. "failed_manual" — тот же путь, что
          // и у любого другого сбоя выдачи: admin_note, уведомление
          // продавцу, и честный текст покупателю ("часть вышлет вручную")
          // вместо "выдан".
          itemOutcome = "failed_manual";
          failReason = "у товара нет ни одного прикреплённого файла";
