...
export async function smartSearchProductIds(
  ...
  // Одинаковый нейтральный "товар в интернет-магазине" не давал модели домена,
  // хотя обе ниши платформы совсем разные по тому, что и зачем ищут: учитель
  // формулирует запрос через предмет/класс/тему урока ("тесты по алгебре для
  // 7 класса"), покупатель кондитерской — через повод/начинку/оформление
  // ("что-то на день рождения пятилетке"). Явная формулировка домена в
  // промпте — то же, что и попытка человека угадать контекст магазина по
  // одному отвлечённому запросу, только для модели это разница между "понял
  // с полуслова" и "неверно интерпретировал терминологию".
  const domain =
    currentVertical() === "confectionery"
