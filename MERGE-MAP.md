...
> **⚠ Поправка после первого прохода.** «Эталон» ниже выбирался по дате
> последнего изменения — эвристика оказалась неверной чаще, чем ожидалось.
> На проверке 15 файлов с 2–3 версиями свежая дата действительно значила
> «богаче» только в трёх случаях (`products.functions.ts` — мультифайловые
> материалы; `payment-methods.functions.ts` + `admin.payment-methods.tsx` —
> QR-код оплаты). В остальных двенадцати — `categories.functions.ts` (флаг
> видимости + дерево категорий), `settings.functions.ts` (юридические
> документы, RoboKassa), `currency.server.ts` (доп. кеш в памяти),
> `admin.index.tsx` (дашборд одним запросом вместо загрузки всех заказов),
> `api/public/img/\$.ts` (три бакета вместо одного) и других — версия
> Анастасии была старше по дате, но богаче по коду: у неё те же фичи плюс
> собственные модули, добавленные позже без обновления даты остального файла.
> Слепое копирование по дате отменило бы эти фичи. **Перед копированием
> эталона — читайте diff, не только дату.**
