...
describe("выгрузка 1С", () => {
  ...
  it("пустой остаток в отчёте об остатках — это не «в наличии»", () => {
    // Сверка с боевым каталогом показала ровно одну такую позицию. Обещать
    // по ней наличие нельзя: бот предложит то, чего на складе нет.
    const csv = [
      "Номенклатура;;;;;Розничная;;",
      ";;;;;Цена;;Остаток",
      "Покрытие на стол Topfit 110 см;;;;;35,000   KZT;;",
      "Плед шерстяной;;;;;70,000   KZT;;4",
    ].join("\n");
    const res = parseCatalogCsv(csv);
    expect(res.products.map((p) => [p.name.slice(0, 12), p.stock])).toEqual([
      ["Покрытие на ", false],
      ["Плед шерстян", true],
    ]);
  });

  it("прайс без колонки остатка считает всё доступным", () => {
    const csv = "Номенклатура;Цена\nПлед;70000";
    expect(parseCatalogCsv(csv).products[0].stock).toBe(true);
  });
