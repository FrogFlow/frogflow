...
| `/mini-app/library`             | Купленные материалы и повторная выдача файлов       |
...
| `/api/public/mini-app/library`  | Купленные материалы                                 |
| `/api/public/mini-app/orders`   | Список заказов, повторная выдача файлов и оценка    |
...
той же фразой, что бот (`«4 класс»`, а не отдельно «4» и «класс»). Если
совпадений нет и включён умный поиск, тот же LLM-фолбэк работает через
`/api/public/mini-app/search`. Расход за день виден в «Настройках».
...
3. При `delivery_lang_timing=before` — язык цифрового материала до заказа.
   Для физической корзины этот шаг пропускается: файлов нет.
...
После оформления заказа Mini App не закрывается сам: показывает ссылку
в бот (цифровые файлы приходят туда), кнопку «Мои материалы» / «Мои заказы»
и остаётся открытым. Внизу — вкладки Каталог / Материалы / Заказы.

Карточки материалов в каталоге — без описания, с компактными
чипами RU/KZ и сеткой 2 колонки на телефоне / 3 на широком экране.
Фильтр `mlang` оставляет только материалы выбранного языка. Сортировка:
каталог, популярные, новые, цена. Кнопка «В корзине» открывает корзину
повторным нажатием; купленный материал помечается «Куплено» и ведёт
в библиотеку. В корзине — превью обложки.

`/mini-app/library` — купленные цифровые материалы с повторной выдачей
файлов в бот. На карточке товара: список файлов по языкам, отзывы,
похожие материалы из той же папки и «Поделиться».

Цифровая позиция в корзине без «+»: второй экземпляр материала не нужен.

Выбор «все языки» в checkout использует ту же подпись, что бот:
«цена ×N».

В «Моих заказах» физические позиции показывают самовывоз/доставку,
дату, предоплату и не предлагают «получить файлы повторно».

Карточки и PDP физических товаров показывают срок изготовления
(`lead_time_days`) или «в наличии».
