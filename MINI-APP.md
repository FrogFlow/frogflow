...
После оформления цифрового заказа Mini App показывает ссылку в бот
(файлы приходят туда) и закрывает WebView через 4 секунды. Физический
заказ не закрывается сам — покупатель видит подтверждение и может
открыть «Мои заказы».

Карточки дидактических материалов показывают флаги доступных языков
(как в админке). На PDP — языки полностью и подсказка, что файл придёт
в бот после оплаты. Цифровая позиция в корзине без «+»: второй экземпляр
материала не нужен.
