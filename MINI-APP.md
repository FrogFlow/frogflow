...
После оформления физического заказа Mini App не закрывается сама —
покупатель видит подтверждение и может открыть «Мои заказы». Цифровой
заказ по-прежнему закрывает WebView через пару секунд, чтобы файлы
пришли в чат с ботом.
