...
в бот (цифровые файлы приходят туда), кнопку «Мои заказы» и остаётся
открытым. Карточки материалов в каталоге — без описания, с компактными
чипами RU/KZ и сеткой 2 колонки на телефоне / 3 на широком экране.
Фильтр `mlang` оставляет только материалы выбранного языка. Кнопка
«В корзине» открывает корзину повторным нажатием. В корзине — превью
обложки. В «Моих заказах» у цифровых — подсказка про файлы в боте,
язык выдачи и оценка звёздами (модуль `review_request`).
